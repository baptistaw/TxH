// scripts/cleanup-duplicate-preops-auto.js
...
async function cleanupDuplicates() {
  try {
    console.log('🧹 LIMPIEZA AUTOMÁTICA DE EVALUACIONES PREOPERATORIAS DUPLICADAS\n');
    ...
    console.log('🔍 EJEMPLOS DE EVALUACIONES A ELIMINAR (primeras 5):\n');
    ...
    console.log('🔍 EJEMPLOS DE EVALUACIONES A CONSERVAR (primeras 5):\n');
    toKeep.slice(0, 5).forEach((item, index) => {
      console.log(`${index + 1}. ${item.patientName} (CI: ${item.patientId})`);
      console.log(`   ID: ${item.id}`);
      console.log(`   Clínico: ${item.clinician}`);
    ...
    // Verify results
    const remainingPreops = await prisma.preopEvaluation.findMany({
      select: {
        patientId: true
      }
    });

    const patientCounts = remainingPreops.reduce((acc, preop) => {
      acc[preop.patientId] = (acc[preop.patientId] || 0) + 1;
      return acc;
    }, {});

    const patientsStillWithMultiple = Object.values(patientCounts).filter(count => count > 1).length;

    console.log(`📊 Verificación post-limpieza:`);
    console.log(`  - Total evaluaciones en BD: ${remainingPreops.length}`);
    console.log(`  - Pacientes con múltiples evaluaciones: ${patientsStillWithMultiple}\n`);

    if (patientsStillWithMultiple > 0) {
      console.log(`⚠️  Aún quedan ${patientsStillWithMultiple} pacientes con múltiples evaluaciones.`);
      console.log(`Esto puede ser legítimo si tienen evaluaciones diferentes en el tiempo.\n`);
    } else {
      console.log(`✅ No quedan duplicados. Todos los pacientes tienen una única evaluación preoperatoria.\n`);
    }
